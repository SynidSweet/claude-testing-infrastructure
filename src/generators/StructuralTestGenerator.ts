 ...
 * ${usesESModules ? 'ES Modules Mode' : 'CommonJS Mode'}
 */
...
${reactConfig}`;
    ...
    // For ES modules, wrap in async function if needed
    if (usesESModules && (hasReact || imports.includes('import'))) {
      return `// ES Modules setup file requires top-level await
${setupContent}`;
    }
...
${usesESModules ? '// ES Modules compatible setup' : '// CommonJS setup'}
...
beforeAll(() => {
  console.error = (...args) => {
    // Suppress specific React warnings that are not relevant in tests
...
// Mock React.useLayoutEffect to useEffect in test environment
${
  usesESModules
...
`;
  ...
  private getSetupFileName(): string {
    ...
    // Use .mjs for ES modules projects
    const usesESModules = this.analysis.moduleSystem.type === 'esm';

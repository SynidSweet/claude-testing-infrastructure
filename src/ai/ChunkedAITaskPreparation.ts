...
export class ChunkedAITaskPreparation {
  ...
  constructor(
    ...
    if (config.maxConcurrentTasks !== undefined)
      baseConfig.maxConcurrentTasks = config.maxConcurrentTasks;
    if (config.minComplexityForAI !== undefined)
      baseConfig.minComplexityForAI = config.minComplexityForAI;
  ...
  private async createTasksForGap(gap: TestGap): Promise<ChunkedAITask[]> {
    try {
      ...
      if (
        !this.chunkedConfig.enableChunking ||
        estimatedTokens <= this.chunkedConfig.chunkTokenLimit
      ) {
   ...
   */
  private async createSingleTask(gap: TestGap, _sourceCode: string): Promise<ChunkedAITask | null> {
   ...
   */
  private async createChunkedTasks(gap: TestGap, sourceCode: string): Promise<ChunkedAITask[]> {
    const tasks: ChunkedAITask[] = [];
  ...
  private async createChunkTask(
    ...
    try {
      ...
      console.error(
        `Failed to create chunk task for ${gap.sourceFile} chunk ${chunk.chunkIndex}:`,
        error
      );
      return null;
    }
   ...
   */
  private buildChunkedPrompt(gap: TestGap, context: TaskContext, chunk: FileChunk): string {
...
Note: These are the complete existing tests, write new tests that complement these.`

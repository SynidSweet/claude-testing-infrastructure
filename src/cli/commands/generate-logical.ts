...
interface GenerateLogicalOptions extends StandardCliOptions {
  gapReport?: string;
  model?: string;
  concurrent?: string;
  budget?: string;
...
export const generateLogicalCommand = new Command()
  ...
  .action(async (projectPath: string, options: GenerateLogicalOptions, command: Command) => {
    await executeCommand(
      ...
      async (context: CommandContext) => {
        ...
        try {
          ...
          console.log(
            `Total estimated cost: ${chalk.green(`$${costReport.totalCost.toFixed(2)}`)}`
          );
          ...
          const timeoutMs = parseInt(options.timeout || '900') * 1000;
          const orchestrator = new ClaudeOrchestrator({
            maxConcurrent: parseInt(options.concurrent || '3') || 3,
            model: options.model || 'sonnet', // Use alias directly (opus, sonnet, haiku)
      ...
      {
        commandName: 'generate-logical',
        loadingText: 'Initializing AI test generation...',

...
export const generateLogicalBatchCommand = new Command()
  ...
  .option('-b, --batch-size <number>', 'Number of tests to generate per batch (default: 5)', '5')
  .option('-m, --model <model>', 'Claude model to use (opus, sonnet, haiku)', 'sonnet')
  .option('-c, --concurrent <number>', 'Max concurrent Claude processes (default: 2, only used with --allow-concurrent)', '2')
  .option('--allow-concurrent', 'Allow concurrent processing (default: sequential to prevent usage spikes)')
    ...
    try {
      ...
      if (options.resume) {
        ...
        const config: BatchConfig = {
          batchSize,
          model: options.model,
          // Default to sequential (1) unless --allow-concurrent is specified
          maxConcurrent: options.allowConcurrent ? (parseInt(options.concurrent) || 2) : 1,
          timeout: parseInt(options.timeout) * 1000,
          minComplexity: parseInt(options.minComplexity) || 5,
        };

        // Log concurrency warning if not explicitly allowed
        if (!options.allowConcurrent && parseInt(options.concurrent) > 1) {
          console.log(chalk.yellow('⚠️  Running in sequential mode to prevent Claude usage spikes.'));
          console.log(chalk.yellow('   Use --allow-concurrent flag to enable concurrent processing.'));
        }

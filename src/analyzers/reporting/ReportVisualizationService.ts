...
`;
  ...
  generatePriorityChart(priorities: Record<string, number>): string {
    ...
    const chartWidth = Math.min(40, this.visualConfig.terminalWidth ? this.visualConfig.terminalWidth - 30 : 50);
  ...
  generateSummaryTable(summary: GapReportSchema['summary']): string {
    const data = [
      ['Metric', 'Value', 'Status'],
      ['─'.repeat(25), '─'.repeat(10), '─'.repeat(10)],
      ['Total Files Analyzed', summary.totalFiles.toString(), '📁'],
      ['Files with Tests', summary.filesWithTests.toString(), '✅'],
      ['Need Logical Tests', summary.filesNeedingLogicalTests.toString(), summary.filesNeedingLogicalTests > 0 ? '⚠️' : '✅'],
      ['Total Gaps', summary.totalGaps.toString(), this.getGapStatusIcon(summary.totalGaps)],
      ['Assessment', summary.overallAssessment.toUpperCase(), this.getAssessmentIcon(summary.overallAssessment)]
    ];

    let table = '';
    data.forEach(row => {
      table += `${row[0]?.padEnd(25) || ''} │ ${row[1]?.padEnd(10) || ''} │ ${row[2] || ''}\n`;
    });
...
💰 Cost Analysis
${'─'.repeat(20)}
🎯 Tasks: ${cost.numberOfTasks}
🪙 Tokens: ${cost.estimatedTokens.toLocaleString()}

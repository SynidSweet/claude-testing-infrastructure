...
import { ConfigurationLoadResult } from '../config/ConfigurationService';
...
export function displayConfigurationSources(result: ConfigurationLoadResult): void {
  ...
  console.log(`  • Valid configuration: ${result.valid ? chalk.green('✓ Yes') : chalk.red('✗ No')}`);
...
export function createConfigDebugReport(result: ConfigurationLoadResult): string {
  ...
  result.sources.forEach((source: any) => {
    ...
    if (source.errors.length > 0) {
      ...
      source.errors.forEach((err: any) => report.push(`      - ${typeof err === 'string' ? err : err.message}`));
    }
    // ConfigurationSource doesn't have warnings property
    if (source.data) {

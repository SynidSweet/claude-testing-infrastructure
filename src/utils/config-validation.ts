...
export class ConfigurationManager {
    ...
    if (userConfig.generation !== undefined) {
      ...
      if (userConfig.generation.batchSize !== undefined) {
        if (
          ...
          // Add warning for values that might cause usage spikes
          if (userConfig.generation.batchSize > 5) {
            const warning = ConfigErrorFormatter.formatError({
              field: 'generation.batchSize',
              value: userConfig.generation.batchSize,
              message: 'High batch size may cause rapid Claude usage depletion',
              suggestion: 'Recommended batch size is 5 or lower to prevent usage spikes',
              example: '5',
            });
            warnings.push(warning);
          }
  ...
  private validateAndMergeAI(
    ...
    if (userConfig.ai !== undefined) {
      ...
      // Validate maxConcurrent (if added to AI config)
      if ((userConfig.ai as any).maxConcurrent !== undefined) {
        const value = (userConfig.ai as any).maxConcurrent;
        if (!Number.isInteger(value) || value < 1 || value > 10) {
          const warning = ConfigErrorFormatter.formatError({
            field: 'ai.maxConcurrent',
            value: value,
            message: 'Value should be between 1 and 10',
            suggestion: 'Use a value of 2 or lower to prevent Claude usage spikes',
            example: '2',
          });
          warnings.push(warning);
        } else if (value > 2) {
          const warning = ConfigErrorFormatter.formatError({
            field: 'ai.maxConcurrent',
            value: value,
            message: 'High concurrent processes may cause rapid Claude usage depletion',
            suggestion: 'Recommended value is 2 or lower to prevent usage spikes',
            example: '2',
          });
          warnings.push(warning);
        }

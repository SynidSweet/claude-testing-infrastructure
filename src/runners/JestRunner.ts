  ...
  protected async executeTests(): Promise<TestResult> {
    ...
    return new Promise(async (resolve) => {
      ...
      if (!reservation.success) {
        logger.error(`Cannot spawn Jest process: ${reservation.reason}`);
        resolve({
          success: false,
          exitCode: 1,
          testSuites: 0,
          ...
          failures: [
            {
              ...
              test: 'Process Limit',
              message: `Cannot spawn Jest process: ${reservation.reason}`,
            },
          ],
          output: '',
      ...
      if (!registration.success) {
        logger.error(`Failed to register Jest process: ${registration.reason}`);
        child.kill('SIGTERM');
        resolve({
          success: false,
          exitCode: 1,
